...
> [!NOTE]
> For tasks that are just submitted to Batch, DeleteTask API call needs up to 10 minutes to take effect. Before it takes effect, some other tasks might be prevented from being scheduled. It's because Batch Scheduler still tries to schedule the tasks just deleted. If you want to delete one task shortly after it’s submitted, please terminate the task instead (since terminate task will take effect immediately). And then delete the task 10 minutes later.
